...
A shortcut can save an optional single-line instruction such as `You are a Realtime Voice Agent Architect`. Each later use of that shortcut sends the saved instruction through `byok-runtime`'s provider-native instruction channel while keeping the caller's text as the user prompt.
...
- **Instruction input is single-line.** (session-settled: user-approved — chosen over a multiline editor to keep this increment aligned with the existing prompt interface.) Governs R1.
...
- KTD3. **Use a custom instruction prompt and one canonical validation path.** (session-settled: user-approved — chosen over the stock Clack text renderer: it leaves submitted values in terminal frames.) Add `@clack/core` 1.4.3 as a direct dependency and implement an instruction-specific `TextPrompt` renderer. It shows input only in the active editing state, retains the resolved value, and clears the internal input before submit, retry, cancel, error, and settled frames render. Producers map whitespace-only input to absence and preserve accepted nonblank text byte-for-byte. Credential validation remains application-level so the prompt never enters a value-bearing error state. The persistence boundary enforces the same record invariant and repeats the persistence-blocking check before serialization or temporary-file creation. This enforces R1-R3.
...
- **Test scenarios:**
  ...
  - Reject blank-only, multiline, control-bearing, malformed, and persistence-blocking credential-bearing stored values without echoing them.
...
- **Approach:**
  ...
  2. Add `@clack/core` 1.4.3 as a direct dependency and implement KTD3's instruction-specific `TextPrompt` mode. Retain the resolved value before clearing internal input so later frames cannot render it.
