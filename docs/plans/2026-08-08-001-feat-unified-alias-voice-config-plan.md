...
- R8. Malformed or unsupported unified configuration must fail before alias mutation, generation, speech, or replacement of any valid on-disk document.
...
- Configuration maintenance actions are deterministic and noninteractive. They must not invoke providers, credentials, generation, speech, or stdin.
...
- **Approach:**
  ...
  5. Move voice configuration validation before any speech call and keep unsupported-platform rejection ahead of voice execution.
...
- **Test scenarios:**
  ...
  - With an invalid unified file and valid legacy files, alias, explicit provider/model, and voice generation paths fail before runtime generation or speech and do not fall back.
...
- **Test scenarios:**
  ...
  - Maintenance flags reject every other option or positional, use closed stdin, and perform zero prompt, provider, credential, generation, or speech calls.
