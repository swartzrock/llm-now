...
- **Implementation reconciliation:** `main` adopted unified `config.toml` while this feature branch was in progress. Unified configuration remains version 1 and stores a nonempty `directories` list directly on the alias; its first entry is the primary working directory and later entries are ordered additional roots. `directory_access` may declare `read-only` or `read-write`, defaults to read-only when omitted, and is invalid without `directories`. The strict sticky v3 rules in this plan apply to the legacy `aliases.json` format, which remains readable and migrates older workspaces as read-only without loss.
...
- R4. A workspace must contain exactly one absolute primary directory and an ordered zero-or-more list of absolute additional directories. Its access policy is `read-only` or `read-write`, with omitted `directory_access` defaulting to read-only. The primary directory is the CLI process cwd; additional directories are explicit accessible roots; the effective access policy covers every listed directory.
...
- **Explicit write policy.** (session-settled: user-directed — chosen so write authority is always visible and opt-in while read-only configuration stays simple.) `directories` defaults to read-only when `directory_access` is omitted; `read-write` must be explicit; Codex supports both modes and Claude supports only read-only. Governs R4-R6 and R9-R16.
...
- KTD2. **Keep unified TOML safe by default and legacy v3 compatible.** Unified aliases accept `directories` without `directory_access` as read-only and serialize the effective access explicitly on canonical rewrites. `directory_access` without `directories` is invalid. Legacy v3 records carry `primaryDirectory`, `additionalDirectories`, and `directoryAccess`; pre-access v3 workspaces load as read-only for migration. Copy nested state when persisting, compare ordered roots and access in `sameAliasRecord`, and preserve v1/v2 no-eager-migration behavior.
...
- **Configured roots expose more local data and can permit mutations.** Default omitted `directory_access` to read-only, require an explicit `read-write` grant, use a default-No Codex write warning, retain Codex read-only sandboxing when write is declined, enable only Claude read tools, show access in workspace labels, preflight write permission, and document plaintext paths and child-process visibility.
